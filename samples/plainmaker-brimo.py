...
class BrimoEncryptorDecryptor(IEncryptorDecryptor):
    ...
    def decrypt_http_response(self, original_response, iResponseInfo):
        res_body = IEncryptorDecryptor.get_http_body(original_response, iResponseInfo)
    ...
    def encrypt_http_response(self, original_response, iResponseInfo):
        ...
        integrity_check = hashlib.md5(res_body).hexdigest().encode()
        nonce = integrity_check[:8].decode() + ('0' * (16-8 - len(self.saved_increment))) + self.saved_increment
        ...
        encrypted = cipher.doFinal(res_body)
        encrypted = encrypted.tostring()
        encrypted = b64encode(encrypted)
        print("Encrypted Response", encrypted)

        full_response = '"%s"' % (integrity_check + encrypted).decode().replace('/','\/')
