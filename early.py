...
class IEncryptorDecryptor():
    ...
    def plainify_http_request(self, plain_request, iRequestInfo):
        ...
        req_variables = self.generate_http_request_variables(req_uri, req_method)
        request = self.construct_tampered_request(plain_request, iRequestInfo, req_variables)
    ...
    def construct_tampered_request(self, plain_request, iRequestInfo, req_variables):
    ...
    def construct_tampered_response(self, plain_request, iRequestInfo, req_variables):
        pass
...
class SenyumEncryptorDecryptor(IEncryptorDecryptor):
    ...
    def generate_http_request_variables(self, uri, body="", timestamp=None, method="POST"):        
        v = uri
        x = method.upper()
    ...
    def plainify_http_request(self, plain_request, iRequestInfo):
        req_uri = SenyumEncryptorDecryptor.get_request_uri(iRequestInfo)
        req_method = SenyumEncryptorDecryptor.get_request_method(iRequestInfo)
        req_body = SenyumEncryptorDecryptor.get_request_body(plain_request, iRequestInfo)

        req_variables = self.generate_http_request_variables(uri=req_uri, body=req_body, method=req_method)
        return super().construct_tampered_request(plain_request, iRequestInfo, req_variables)
        
    def plainify_http_response(self):
        return super().plainify_http_response()
